...
import com.dongxl.pushdeme.huawei.agent.common.handler.CheckUpdateHandler;
import com.dongxl.pushdeme.huawei.agent.common.handler.ConnectHandler;
import com.dongxl.pushdeme.huawei.agent.push.*;
import com.dongxl.pushdeme.huawei.agent.push.handler.*;
...
public class HMSAgent {
    private final static String TAG = HMSAgent.class.getSimpleName();
    /**
     * 基础版本 | Base version
     */
    ...
    public static final class Push {
        ...
        /**
         * 设置标签信息。应用可以调用该接口来设置标签，然后根据指定的标签信息来下发Push消息。
         * 服务已经不存在
         * @param tags    设置的标签键值对map,以key-value方式集合提供
         * @param handler 方法调用结果回调（结果会在主线程回调）
         */
        @Deprecated
        public static void setTopic(Map<String, String> tags, SetTopicHandler handler) {
            new SetTopicApi().setTopic(tags, handler);
        }
         ...
         */
        @Deprecated
        public static void getTopic(GetTopicHandler handler) {
            new GetTopicApi().getTopic(handler);
        }

        /**
         * 删除指定的标签信息
         * 服务已经不存在
         * @param tagKeys 需要删除的标签的key列表
         * @param handler
         */
        @Deprecated
        public static void deleteTopic(List<String> tagKeys, DeleteTopicHandler handler) {
            new DeleteTopicApi().deleteTopic(tagKeys, handler);
        }
